(function() {
  ...
  XMLHttpRequest.prototype.send = function() {
    if (shouldBlock(this._url)) {
      ...
      Object.defineProperty(this, 'statusText', { writable: true, value: 'No Content' });
  ...
  const skipYouTubeAds = () => {
    ...
    const hasAdOverlay = document.querySelector('.ytp-ad-player-overlay, .ytp-ad-overlay-container, .ytp-ad-skip-button-container');
    ...
    if (video && (adShowing || hasAdOverlay)) {
      ...
      const skipButton = document.querySelector('.ytp-ad-skip-button, .ytp-ad-skip-button-modern, .ytp-ad-skip-button-self-modern');
      if (skipButton) {
        skipButton.click();
  ...
  const muteSpotifyAds = () => {
    ...
    const adLink = document.querySelector('[data-testid="now-playing-widget"] a[href*="/ad/"]');
    const isAdPlaying = adLink || (nowPlayingWidget && nowPlayingWidget.innerText.toLowerCase().includes('advertisement'));
